...
class LoggerEvalCallback(EventCallback):
    ...
    logger = TerminalLogger().getLogger(  # type: ignore
        name='EVALUATION', level=LOG_CALLBACK_LEVEL
    )
    ...
    def __init__(
        ...
        # to avoid this
        self.eval_freq = (
            eval_freq_episodes * train_env.get_wrapper_attr('timestep_per_episode')
            - eval_freq_episodes
        )
        self.save_path = (
            self.train_env.get_wrapper_attr('workspace_path') + '/evaluation'
        )
        ...
        self.evaluation_columns = [
            col
            for col in eval_env.get_wrapper_attr('summary_metrics')
        ...
        if self.wandb_log:
            ...
            self.train_env.get_wrapper_attr('wandb_run').define_metric(
                'Evaluation/*', step_metric='Evaluation/evaluation_num'
            )
    ...
    def _on_event(self) -> None:
        ...
        evaluation_summary = {
            ...
            **{
                key: np.mean(evaluation_episodes[key])
                for key in self.evaluation_columns
                if key in evaluation_episodes
            },
        }
        ...
        evaluation_summary_df = pd.DataFrame([evaluation_summary]).dropna(
            axis=1, how="all"
        )
        ...
        if not evaluation_summary_df.empty:
            self.evaluation_metrics = pd.concat(
                [
                    self.evaluation_metrics.dropna(axis=1, how="all"),
                    evaluation_summary_df,
                ],
                ignore_index=True,
            )
            self.evaluation_metrics.to_csv(
                os.path.join(self.save_path, 'evaluation_metrics.csv')
            )
    ...
    def _sync_envs(self):
        # normalization
        if all(
            is_wrapped(env, NormalizeObservation)
            ...
            get_eval_attr('set_mean')(get_train_attr('mean'))
            get_eval_attr('set_var')(get_train_attr('var'))
            get_eval_attr('set_count')(get_train_attr('count'))
